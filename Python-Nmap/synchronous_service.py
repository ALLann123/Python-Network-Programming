...
def nmap_scan(ip_address, port):
    ...
    if ip_address in portscanner.all_hosts():
        protocol = "tcp"  # We are scanning TCP ports
        # Save the state of the port
        state = portscanner[ip_address]['tcp'][int(port)]['state']
        #lets get the services
        service = portscanner[ip_address][protocol][int(port)].get('name', 'Unknown')

        print("[+] Executing command: ", portscanner.command_line())  # Fixed method name
        print(f"[+] Target:{ip_address} tcp/{port} Service:{service} State: {state}")
    else:
...
└─# python service_synchronous.py
************************************************
           Synchronous Scan 
************************************************
Enter the target IP>> 127.0.0.1
Enter Ports to scan (comma-separated)>> 21,22,80
_______________________________________
[+] Executing command:  nmap -oX - -p 21 -sV 127.0.0.1
[+] Target:127.0.0.1 tcp/21 Service:ftp State: open
[+] Executing command:  nmap -oX - -p 22 -sV 127.0.0.1
[+] Target:127.0.0.1 tcp/22 Service:ssh State: open
[+] Executing command:  nmap -oX - -p 80 -sV 127.0.0.1
[+] Target:127.0.0.1 tcp/80 Service:http State: open
'''
